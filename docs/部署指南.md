# 🚀 汉字学习助手 - 部署指南

## 构建完成 ✅

生产版本已成功构建！构建文件位于 `dist/` 目录。

```
✓ built in 867ms
dist/index.html                   0.61 kB
dist/assets/index-CG94Ls_C.css   19.22 kB
dist/assets/index-Dx5McZHo.js   431.66 kB
```

---

## 📦 部署方式选择

### 方案一：Vercel（推荐）⭐

**特点**：
- ✅ 免费
- ✅ 全球 CDN
- ✅ 自动 HTTPS
- ✅ 一键部署
- ✅ 自动构建

#### 部署步骤

**1. 安装 Vercel CLI**
```bash
npm install -g vercel
```

**2. 登录 Vercel**
```bash
vercel login
```

**3. 部署项目**
```bash
cd /Users/ts/Desktop/HanZi
vercel
```

**4. 按提示操作**
```
? Set up and deploy "~/Desktop/HanZi"? [Y/n] Y
? Which scope do you want to deploy to? [选择你的账号]
? Link to existing project? [N]
? What's your project's name? hanzi-learning
? In which directory is your code located? ./
? Want to override the settings? [N]
```

**5. 部署成功**
```
✅ Production: https://hanzi-learning-xxx.vercel.app
```

#### 自动部署（可选）
1. 在 GitHub 创建仓库
2. 推送代码到 GitHub
3. 在 Vercel 网站导入项目
4. 每次推送自动部署

---

### 方案二：Netlify

**特点**：
- ✅ 免费
- ✅ 全球 CDN
- ✅ 自动 HTTPS
- ✅ 拖拽部署

#### 部署步骤

**方法 1：拖拽部署（最简单）**
1. 访问 https://app.netlify.com/drop
2. 拖拽 `dist` 文件夹到页面
3. 部署完成！

**方法 2：CLI 部署**
```bash
# 安装 Netlify CLI
npm install -g netlify-cli

# 登录
netlify login

# 部署
cd /Users/ts/Desktop/HanZi
netlify deploy --prod
```

**方法 3：GitHub 自动部署**
1. 推送代码到 GitHub
2. 在 Netlify 导入项目
3. 配置构建命令：`yarn build`
4. 配置发布目录：`dist`
5. 自动部署

---

### 方案三：GitHub Pages

**特点**：
- ✅ 免费
- ✅ GitHub 集成
- ✅ 简单可靠

#### 部署步骤

**1. 安装 gh-pages**
```bash
cd /Users/ts/Desktop/HanZi
yarn add -D gh-pages
```

**2. 修改 package.json**
添加部署脚本：
```json
{
  "scripts": {
    "deploy": "yarn build && gh-pages -d dist"
  },
  "homepage": "https://[你的用户名].github.io/hanzi-learning"
}
```

**3. 部署**
```bash
yarn deploy
```

**4. 配置 GitHub Pages**
- 进入仓库 Settings → Pages
- Source 选择 `gh-pages` 分支
- 保存

---

### 方案四：自己的服务器

#### 使用 Nginx

**1. 上传文件**
```bash
scp -r dist/* user@your-server:/var/www/hanzi
```

**2. 配置 Nginx**
```nginx
server {
    listen 80;
    server_name your-domain.com;
    root /var/www/hanzi;
    index index.html;

    location / {
        try_files $uri $uri/ /index.html;
    }

    # Gzip 压缩
    gzip on;
    gzip_types text/css application/javascript application/json;
}
```

**3. 重启 Nginx**
```bash
sudo nginx -t
sudo systemctl reload nginx
```

#### 使用 Apache

**1. 上传文件到服务器**
```bash
scp -r dist/* user@your-server:/var/www/html/hanzi
```

**2. 创建 .htaccess**
```apache
<IfModule mod_rewrite.c>
  RewriteEngine On
  RewriteBase /
  RewriteRule ^index\.html$ - [L]
  RewriteCond %{REQUEST_FILENAME} !-f
  RewriteCond %{REQUEST_FILENAME} !-d
  RewriteRule . /index.html [L]
</IfModule>
```

---

## 🌐 域名配置（可选）

### Vercel 自定义域名
1. 进入项目 Settings → Domains
2. 添加你的域名
3. 配置 DNS：
   ```
   CNAME  www  cname.vercel-dns.com
   A      @    76.76.21.21
   ```

### Netlify 自定义域名
1. 进入 Site settings → Domain management
2. 添加自定义域名
3. 配置 DNS：
   ```
   CNAME  www  [your-site].netlify.app
   ```

---

## 📊 性能优化建议

### 已完成的优化
- ✅ Vite 构建优化
- ✅ 代码压缩
- ✅ CSS 提取和压缩
- ✅ 资源 Tree-shaking

### 可选优化
- [ ] 启用 CDN 加速
- [ ] 配置缓存策略
- [ ] 添加 Service Worker（PWA）

---

## 🔒 环境变量配置

如果需要使用环境变量：

**1. 创建 .env.production**
```bash
VITE_API_URL=https://api.example.com
```

**2. 在代码中使用**
```typescript
const apiUrl = import.meta.env.VITE_API_URL
```

**3. 部署平台配置**
- Vercel: Project Settings → Environment Variables
- Netlify: Site settings → Build & deploy → Environment

---

## 📱 PWA 配置（可选）

将应用变成可安装的 PWA：

**1. 安装插件**
```bash
yarn add -D vite-plugin-pwa
```

**2. 配置 vite.config.ts**
```typescript
import { VitePWA } from 'vite-plugin-pwa'

export default defineConfig({
  plugins: [
    react(),
    VitePWA({
      registerType: 'autoUpdate',
      manifest: {
        name: '汉字学习助手',
        short_name: '汉字学习',
        description: '小学生汉字学习工具',
        theme_color: '#FCD34D',
        icons: [
          {
            src: 'icon-192.png',
            sizes: '192x192',
            type: 'image/png'
          }
        ]
      }
    })
  ]
})
```

---

## 🧪 本地预览

在部署前测试生产构建：

```bash
# 预览构建结果
yarn preview

# 或使用 serve
npx serve dist
```

访问 http://localhost:4173 查看效果。

---

## 📋 部署检查清单

### 部署前
- [x] TypeScript 编译无错误
- [x] 生产构建成功
- [x] 本地预览正常
- [ ] 所有功能测试通过
- [ ] 移动端适配正常
- [ ] 性能测试合格

### 部署后
- [ ] 网站可访问
- [ ] HTTPS 正常
- [ ] 路由跳转正常
- [ ] 手写功能正常
- [ ] 笔顺动画正常
- [ ] iPad 适配正常

---

## 🔧 常见问题

### 1. 路由 404 错误
**问题**：刷新页面出现 404
**解决**：配置重定向规则（已包含在 vercel.json 和 netlify.toml）

### 2. 资源加载失败
**问题**：CSS/JS 加载 404
**解决**：检查 base 路径配置

### 3. 字体加载问题
**问题**：楷体不显示
**解决**：添加字体回退：
```css
font-family: 'STKaiti', 'KaiTi', 'SimKai', serif;
```

### 4. Canvas 不工作
**问题**：手写功能失效
**解决**：检查 HTTPS，某些功能需要安全上下文

---

## 📈 监控和分析（可选）

### Google Analytics
```html
<!-- index.html -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-XXXXXXXXXX"></script>
```

### Vercel Analytics
自动集成，无需配置

---

## 🎯 推荐部署方案

**最简单**：Netlify 拖拽部署  
**最专业**：Vercel + GitHub 自动部署  
**最灵活**：自己的服务器  

---

## 🚀 快速部署命令

### Vercel（推荐）
```bash
# 一键部署
vercel --prod
```

### Netlify
```bash
# 一键部署
netlify deploy --prod --dir=dist
```

---

## 📞 获取帮助

- **Vercel 文档**：https://vercel.com/docs
- **Netlify 文档**：https://docs.netlify.com
- **Vite 文档**：https://vitejs.dev

---

## ✅ 部署完成后

1. 测试所有功能
2. 分享链接给用户
3. 收集反馈
4. 持续优化

---

**祝部署顺利！** 🎉

**构建文件位置**：`/Users/ts/Desktop/HanZi/dist`  
**推荐部署平台**：Vercel 或 Netlify  
**预计部署时间**：5-10 分钟

